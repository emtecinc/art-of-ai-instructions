...
Before creating any Salesforce-generic action in an object page, MUST CHECK `pages/SF-Basepage/sf-page.ts` first.
...
export class SalesforcePage extends BasePage {
  ...
  // captureScreenshot — inherited from BasePage (playwright-custom-core), do NOT reimplement
}
```
...
### Navigation Priority
**Navigation Priority (Strict Order):** App Launcher → Global Actions → Global Search → Direct URL
...
- Only if the object cannot be found via the App Launcher, then (and only then) consider the next navigation methods in above priority order. 
...
#### Example Failure:
```typescript
// INVALID — Matches both "Opportunities" AND "Opportunities Recently Viewed"
...
```typescript
// VALID — All strategies match exactly one element
private get listViewHeading() {
  return new ResilientLocator(this['page'], [
...
```
## Action Methods
All action methods are `public async` with `try/catch`. **Every catch block MUST call `captureScreenshot()` before throwing error.**
```typescript
public async fillAccountName(name: string): Promise<void> {
  try {
...
async verifyHeadingContains(expectedText: string): Promise<void> {
  try {
    ...
    await this.captureScreenshot(this.page, test.info(), 'verify-heading-failure');
...
```typescript
await expect(comboboxLocator).toContainText(expectedValue);  // (NEVER toHaveValue)
```
