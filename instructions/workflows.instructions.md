...
async navigateToAccounts(): Promise<void> {
  await this.testStep('Navigate to Accounts list page', async () => {
    ...
    //Capture screenshot after navigation step, as this is an imporant step
...
```typescript
// VALID — Workflow exposes high-level methods, spec calls them directly
// In spec: await workflow.createEntity(entityData);
...
// INVALID — Spec should NOT call granular steps
